...
// Interval
int interval_1 = 5; // watch interval for pat_1 vibration pattern
int interval_2 = 15; // second watch interval for pat_2 vibration pattern

// First Vibe pattern: ON (ms), OFF (ms), ON (ms):
static const uint32_t const segments_1[] = {150, 75, 150, 75, 150};
VibePattern pat_1 = {
    .durations = segments_1,
    .num_segments = ARRAY_LENGTH(segments_1),
};

// Second Vibe pattern: ON (ms), OFF (ms), ON (ms):
static const uint32_t const segments_2[] = {150, 75, 150, 75, 300};
VibePattern pat_2 = {
    .durations = segments_2,
    .num_segments = ARRAY_LENGTH(segments_2),
};
...
{    
    ...
    else if(seconds == 59) 
    {
		...
		// Vibrate every interval
		/*
		if (mins % interval_2 == interval_2 - 1) {
			vibes_enqueue_custom_pattern(pat_2);
		} else 
		*/
			if (mins % interval_1 == interval_1 - 1) {
			vibes_enqueue_custom_pattern(pat_1);
		} 
